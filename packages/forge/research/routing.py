...
VOCABULARY: dict[str, dict[str, tuple[str, ...]]] = {
    "by_hour": {
        "strong": (
            "hour", "hourly", "time of day", "session", "morning", "afternoon",
            "london open", "new york open", "overnight", "intraday timing",
            "what time", "which hours", "open", "close",
        ),
    ...
    "by_volatility_percentile": {
        "strong": (
            "volatility", "volatile", "atr", "vol regime", "quiet market",
            "choppy", "calm", "percentile", "vix",
        ),
        "weak": ("regime", "conditions", "range"),
    },
    "hour_by_volatility": {
        "strong": (
            "surface", "3d", "three dimensional", "both", "interaction",
            "time and volatility", "hour and volatility", "combination",
            "where does the edge live", "jointly",
        ),
        "weak": ("map", "landscape", "heatmap", "together", "across"),
    },
    "edge_over_time": {
        "strong": (
            "decay", "decayed", "still work", "stop working", "stopped working",
            "over time",
            "by month", "monthly", "since", "after 20", "before 20", "recent",
            "degraded", "deteriorat", "getting worse", "held up",
        ),
        "weak": ("year", "years", "history", "period", "trend"),
    },
    "excursion": {
        "strong": (
            "mfe", "mae", "excursion", "giving back", "give back",
            "stopped out", "stop too tight", "target too far", "run up",
            "drawdown per trade", "unrealised", "unrealized",
        ),
    ...
    "worst_decile": {
        "strong": (
            "worst", "biggest losers", "largest losses", "bad trades",
            "losing trades", "what went wrong", "worst 10", "worst decile",
            "responsible for the losses", "blow up", "blew up",
        ),
