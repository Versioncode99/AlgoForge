...
export function App() {
  ...
  return <div className="shell">
    <aside>
      ...
      <nav aria-label="Primary">{tabs.map(item => <button key={item} aria-label={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item === 'Prop Firm' ? <ShieldCheck/> : item === 'Agents' ? <Bot/> : item === 'Evolution' ? <GitBranch/> : <ChartNoAxesCombined/>}<span>{item}</span></button>)}</nav>
